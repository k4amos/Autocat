...
do
    ...
    then
        ...
        #echo "$nb_digits"
        timeout --foreground 3600 hashcat -m 1000 -a 3 -1 ?l?d?u -2 ?l?d -3 $mask 3_default.hcchr lyon_ntds.hash --status --status-timer 1 --machine-readable -O | tee "report_lyon_optimal/$i.log"
    else
        ...
        if [ -f "/dico/$wordlist"]
        then
            timeout --foreground 3600 hashcat -m 1000 lyon_ntds.hash /dico/$wordlist -r /dico/rules/$rule --status --status-timer 1 --machine-readable -O | tee "report_lyon_optimal/$i $j.log"
        else
            timeout --foreground 3600 hashcat -m 1000 lyon_ntds.hash concat_all_ntds wordlists/$wordlist -r /dico/rules/$rule --status --status-timer 1 --machine-readable -O | tee "report_lyon_optimal/$i $j.log"
        fi
    fi
done
